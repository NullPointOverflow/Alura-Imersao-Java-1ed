...
public class MarvelStickerFactory extends StickerFactory implements Arredondamento {

	private BufferedImage logoMarvel;
	...
	public MarvelStickerFactory() {

		try (InputStream psudoLogo = MarvelStickerFactory.class.getClassLoader()
				.getResourceAsStream("logo/marvel_logo.png")) {

			this.logoMarvel = ImageIO.read(psudoLogo);

		} catch (IOException | IllegalArgumentException e) {

			this.logoMarvel = Default.getEmptyImage();
	...
	private void salvarSticker(BufferedImage imagem, String titulo) throws IOException {
		...
		String diretorio = super.getDiretorioDeDestino() + "marvel/";
	...
	public Map<String, Integer> gerar(List<Conteudo> listaDeConteudo) {
		...
		for (int i = 0; i < listaDeConteudo.size(); i++) {
			...
			try {
				...
				this.caneta.drawImage(this.logoMarvel,
						this.arredondarValor(
								(this.imagemDefinitiva.getWidth() * 0.5) - (this.logoMarvel.getWidth() * 0.5)),
						525, new Canvas());
