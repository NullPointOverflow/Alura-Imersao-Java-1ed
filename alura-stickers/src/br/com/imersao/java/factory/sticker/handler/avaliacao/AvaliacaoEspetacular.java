...
public class AvaliacaoEspetacular implements AvaliacaoHandler, CalculadorDeImagem {

	private final AvaliacaoHandler proximoHandler;
	...
	public AvaliacaoEspetacular(AvaliacaoHandler proximoHandler) {

		this.proximoHandler = proximoHandler;

		this.avaliacao = Avaliacao.ESPETACULAR;
	...
	public void manipular(FonteFactory geradorDeFonte, BufferedImage imagem, Conteudo conteudo) {
		...
		if (!(this.deveManipular(conteudo.avaliacao()))) {
			...
			this.proximoHandler.manipular(geradorDeFonte, imagem, conteudo);
